...
add_binds("normal", {
    ...
    -- Custom
    key({"Control"}, "I", "Toggle DOM inspector.",
    ...
    key({"Control"}, ",", "Reorder tab left `[count=1]` positions.",
        function (w, m)
            w.tabs:reorder(w.view, w.tabs:current() - m.count)
        end, {count=1}),

    key({"Control"}, ".", "Reorder tab right `[count=1]` positions.",
        function (w, m)
...
add_cmds({
    ...
    cmd("q", "Close the current tab.",
        function (w) w:close_tab() end),

    cmd("qa", "Close the current window.",
        function (w, a, o) w:close_win(o.bang) end),

    cmd("wq", "Save the session and close the current window.",
        --function (w) w:close_tab() end),
        function (w) w:save_session() w:close_win() end),
        --function (w, a, o) w:close_win(o.bang) end),
